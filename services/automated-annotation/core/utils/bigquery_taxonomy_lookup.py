...
def _get_api_env_name() -> str:
    """Map DB_STAGE environment variable to BigQuery dataset name."""
    ...
    env_name = mapping.get(stage, "api")
    logger.info(f"[BigQuery] Mapped DB_STAGE={stage} to API environment: {env_name}")
    return env_name
